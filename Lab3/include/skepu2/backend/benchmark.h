...
namespace skepu2
{
	namespace containerutils
	{
		...
		inline void invalidateDeviceData() {}
		
		template<typename Arg, typename... Args>
		inline void invalidateDeviceData(Arg& arg, Args&&... args)
		{
			arg.invalidateDeviceData();
			invalidateDeviceData(std::forward<Args>(args)...);
		}
		...
		template<typename T, typename... Args>
		inline void reserve(size_t size, skepu2::Matrix<T> &, Args&&... args)
		{
		//	arg.reserve(size);
			reserve(size, std::forward<Args>(args)...);
		}
	...
	namespace benchmark
	{
		...
		inline std::chrono::microseconds measureExecTime(BenchmarkFunc f, size_t size)
		{
		...
		inline std::chrono::microseconds measureExecTime(std::function<void(void)> f)
		{
			auto t1 = std::chrono::high_resolution_clock::now();
			f();
			auto t2 = std::chrono::high_resolution_clock::now();
