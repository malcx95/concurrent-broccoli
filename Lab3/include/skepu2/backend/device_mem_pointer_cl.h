...
namespace skepu2
{
	namespace backend
	{
		...
		public:
			...
			void copyHostToDevice(long numElements = -1, bool copyLast=false) const;
			void copyDeviceToHost(long numElements = -1, bool copyLast=false) const;
			void copyDeviceToDevice(cl_mem copyToPointer, long numElements, size_t dstOffset = 0, size_t srcOffset = 0) const;
		...
		private:
			void copyHostToDevice_internal(T* src, cl_mem dest, long numElements, size_t offset = 0) const;
		 ...
		 */
		template <typename T>
		DeviceMemPointer_CL<T>::DeviceMemPointer_CL(T* root, T* start, size_t numElements, Device_CL* device) : m_effectiveHostDataPointer(start), m_hostDataPointer(root), m_effectiveNumElements(numElements), m_numElements(numElements), m_device(device)
		{
		 ...
		 */
		template <typename T>
		void DeviceMemPointer_CL<T>::copyDeviceToDevice(cl_mem copyToPointer, long numElements, size_t dstOffset, size_t srcOffset) const
		{
 ...
 */
template <typename T>
		void DeviceMemPointer_CL<T>::copyHostToDevice_internal(T* src_ptr, cl_mem dest_ptr, long numElements, size_t offset) const
		{
#ifdef SKEPU_MEASURE_TIME_DISTRIBUTION
#ifdef SKEPU_MEASURE_ONLY_COPY
			clFinish(m_device->getQueue());
	...
	//	m_deviceDataHasChanged = false;
		 ...
		 */
		template <typename T>
		void DeviceMemPointer_CL<T>::copyHostToDevice(long numElements, bool copyLast) const
		{
			cl_int err;
			size_t sizeVec;
			if (numElements == -1)
		 ...
		 */
		template <typename T>
		void DeviceMemPointer_CL<T>::copyDeviceToHost(long numElements, bool copyLast) const
		{
