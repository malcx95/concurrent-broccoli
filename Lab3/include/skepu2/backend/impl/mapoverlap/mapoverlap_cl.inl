...
namespace skepu2
{
	namespace backend
	{
		...
		::mapOverlapSingle_CL(size_t deviceID, Container<Ret>& res, Container<T>& arg, pack_indices<AI...>, pack_indices<CI...>, CallArgs&&... args)
		{
			// Setup parameters
		...
		template<template<class> class Container, size_t... AI, size_t... CI, typename... CallArgs>
		void MapOverlap1D<MapOverlapFunc, CUDAKernel, C2, C3, C4, CLKernel>
		::mapOverlapNumDevices_CL(size_t numDevices, Container<Ret>& res, Container<T>& arg, pack_indices<AI...>, pack_indices<CI...>, CallArgs&&... args)
		{
		...
		void MapOverlap1D<MapOverlapFunc, CUDAKernel, C2, C3, C4, CLKernel>
		::mapOverlapSingle_CL_Row(size_t deviceID, Matrix<Ret>& res, Matrix<T>& arg, pack_indices<AI...>, pack_indices<CI...>, CallArgs&&... args)
		{
			Device_CL *device = this->m_environment->m_devices_CL[deviceID];
		...
		template<size_t... AI, size_t... CI, typename... CallArgs>
		void MapOverlap1D<MapOverlapFunc, CUDAKernel, C2, C3, C4, CLKernel>
		::mapOverlapSingle_CL_RowMulti(size_t numDevices, Matrix<Ret>& res, Matrix<T>& arg, pack_indices<AI...>, pack_indices<CI...>, CallArgs&&... args)
		{
			const size_t overlap = this->m_overlap;
			const size_t numrows = arg.total_rows();
		...
		template<size_t... AI, size_t... CI, typename... CallArgs>
		void MapOverlap1D<MapOverlapFunc, CUDAKernel, C2, C3, C4, CLKernel>
		::mapOverlapSingle_CL_Col(size_t deviceID, Matrix<Ret>& res, Matrix<T>& arg, pack_indices<AI...>, pack_indices<CI...>, CallArgs&&... args)
		{
			Device_CL *device = this->m_environment->m_devices_CL[deviceID];
			const size_t n = arg.size();
			const size_t overlap = this->m_overlap;
		...
		*  Used internally by other methods to apply column-wise mapoverlap operation.
		...
		template<size_t... AI, size_t... CI, typename... CallArgs>
		void MapOverlap1D<MapOverlapFunc, CUDAKernel, C2, C3, C4, CLKernel>
		::mapOverlapSingle_CL_ColMulti(size_t numDevices, Matrix<Ret>& res, Matrix<T>& arg, pack_indices<AI...>, pack_indices<CI...>, CallArgs&&... args)
		{
			const size_t overlap = this->m_overlap;
			const size_t colWidth = arg.total_rows();
			const size_t numCols = arg.total_cols();
		...
		::mapOverlapSingleThread_CL(size_t deviceID, skepu2::Matrix<Ret>& res, skepu2::Matrix<T>& arg, pack_indices<AI...>, pack_indices<CI...>,  CallArgs&&... args)
		{
			...
			numBlocks[0] = ((out_cols + numThreads[0] - 1) / numThreads[0]) * numThreads[0];
			numBlocks[1] = ((out_rows + numThreads[1] - 1) / numThreads[1]) * numThreads[1];
		...
		::mapOverlapMultipleThread_CL(size_t numDevices, skepu2::Matrix<Ret>& res, skepu2::Matrix<T>& arg, pack_indices<AI...>, pack_indices<CI...>,  CallArgs&&... args)
		{
			...
			for (size_t i = 0; i < numDevices; ++i)
			{
				...
				numThreads[1] = std::min(out_rows, maxThreads / 16);
				numBlocks[0] = ((out_cols + numThreads[0] - 1) / numThreads[0]) * numThreads[0];
				numBlocks[1] = ((outRows + numThreads[1] - 1) / numThreads[1]) * numThreads[1];
