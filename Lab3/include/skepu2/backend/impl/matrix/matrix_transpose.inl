...
// -------------------------------------------------------
	...
	{
		Matrix<T>::createOpenCLProgramForMatrixTranspose();
		...
		backend::Device_CL *device = Matrix<T>::transposeKernels_CL.at(deviceID).second;
		cl_kernel kernel = Matrix<T>::transposeKernels_CL.at(deviceID).first;
